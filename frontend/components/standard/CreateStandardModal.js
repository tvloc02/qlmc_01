...
const CreateStandardModal = ({ isOpen, onClose, onSuccess, programs, organizations }) => {
    ...
    const validateForm = () => {
        ...
        if (!formData.programId) {
            newErrors.programId = 'Chương trình đánh giá là bắt buộc'
        }

        if (!formData.organizationId) {
            newErrors.organizationId = 'Tổ chức - Cấp đánh giá là bắt buộc'
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await standardApi.createStandard(submitData)
            toast.success('Tạo tiêu chuẩn thành công')
            onSuccess()
        } catch (error) {
            console.error('Create standard error:', error)
            const message = error.response?.data?.message || 'Lỗi khi tạo tiêu chuẩn'
            toast.error(message)
        } finally {
            setLoading(false)
        }
            ...
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                Chương trình đánh giá <span className="text-red-500">*</span>
                            </label>
                            <select
                                name="programId"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                Tổ chức - Cấp đánh giá <span className="text-red-500">*</span>
                            </label>
                            <select
                                name="organizationId"
                  ...
                  />
                    ...
                    <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
                        ...
                        >
                            {loading ? 'Đang tạo...' : 'Tạo tiêu chuẩn'}
                        </button>
                    </div>
                </form>
            </div>
